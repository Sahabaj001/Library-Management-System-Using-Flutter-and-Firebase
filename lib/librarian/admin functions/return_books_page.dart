...
class _ReturnBooksPageState extends State<ReturnBooksPage> {
  ...
  Future<void> _returnBook() async {
    ...
    try {
      ...
      // Find the borrowed book by userId and isbn
      ...
      final bookQuery = await booksRef.where('isbn', isEqualTo: isbn).limit(1).get();
      ...
      // Remove ISBN from student's borrowedBooks list
      await FirebaseFirestore.instance
          .collection('students')
          .doc(studentId)
          .update({
        'borrowedBooks': FieldValue.arrayRemove([isbn]),
      });
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(_isProcessing ? 'Processing...' : 'Return Book'),
